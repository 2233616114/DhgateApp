...
class DhpayService {
	...
	function send_goods_confirm_by_platform($para_temp) {
		...
		$html_text = $dhpaySubmit->sendPostInfo($para_temp, $this->payment_gateway, $this->dhpay_config);
	...
	function dhpay_redirect($para_temp) {
		...
		//生成表单提交HTML文本信息
		$dhpaySubmit = new DhpaySubmit();
		$html_text = $dhpaySubmit->buildForm($para_temp, $this->payment_gateway, "get", $button_name,$this->dhpay_config);
	...
	function query_timestamp() {
		$url = $this->payment_gateway."service=query_timestamp&partner=".trim($this->dhpay_config['partner']);
		$encrypt_key = "";
     ...
     */
	function dhpay_iframe($para_temp) {
		//获取远程数据
		$button_name = "Confirm";
		$dhpaySubmit = new DhpaySubmit();
		...
		//$dhpaySubmit->buildForm($para_temp, $this->dhpay_gateway, "get", $button_name,$this->dhpay_config);
		//2.构造模拟远程HTTP的POST请求，获取支付宝的返回XML处理结果:
		//注意：若要使用远程HTTP获取数据，必须开通SSL服务，该服务请找到php.ini配置文件设置开启，建议与您的网络管理员联系解决。
		//$dhpaySubmit->sendPostInfo($para_temp, $this->dhpay_gateway, $this->dhpay_config);
