	...
	<div class="primary_block row">
		...
		<div class="pb-left-column col-xs-12 col-sm-4 col-md-5">
			...
			<div id="image-block" class="clearfix">
				{if $product->new}
					<span class="new-box hidden-xs">
						<span class="new-label">{l s='New'}</span>
					</span>
				{/if}
				{if $product->on_sale}
					<span class="sale-box no-print hidden-xs">
						<span class="sale-label">{l s='Sale!'}</span>
					</span>
				{elseif $product->specificPrice && $product->specificPrice.reduction && $productPriceWithoutReduction > $productPrice}
					<span class="discount hidden-xs">{l s='Reduced price!'}</span>
				{/if}
				{if $have_image}
					<span id="view_full_size" class="hidden-xs">
					...
					<span id="view_full_size" class="visible-xs">
						{if isset($images)}
				...
				{else}
					<span id="view_full_size">
						...
						{if !$content_only}
							<span class="span_link hidden-xs">
								{l s='View larger'}
							</span>
						{/if}
					</span>
			...
			{if isset($images) && count($images) > 0}
				<!-- thumbnails -->
				<div id="views_block" class="hidden-xs clearfix {if isset($images) && count($images) < 2}hidden{/if}">
		...
		<div class="pb-center-column col-xs-12 col-sm-4">
			{if $product->online_only}
				<p class="online_only hidden-xs">{l s='Online only'}</p>
			{/if}
			...
			<p id="product_condition" class="hidden-xs">
			...
			{if !$content_only}
				<!-- usefull links-->
				{*hide*}
				{*<ul id="usefull_link_block" class="hidden-xs clearfix no-print">*}
				...
				<div class="box-info-product">
					<div class="content_prices clearfix">
						{if $product->show_price && !isset($restricted_country_mode) && !$PS_CATALOG_MODE}
							<!-- prices -->
							<div>
								<p class="our_price_display col-xs-3 col-md-12" itemprop="offers" itemscope itemtype="https://schema.org/Offer">{strip}
										...
										{/if}
									{/strip}</p>
								<p id="old_price" class="col-xs-3 col-md-4" {if (!$product->specificPrice || !$product->specificPrice.reduction)} class="hidden"{/if}>{strip}
										{if $priceDisplay >= 0 && $priceDisplay <= 2}
										...
										{/if}
									{/strip}</p>
								<p id="reduction_percent" class="col-xs-3 col-md-6" {if $productPriceWithoutReduction <= 0 || !$product->specificPrice || $product->specificPrice.reduction_type != 'percentage'} style="display:none;"{/if}>{strip}
									<span id="reduction_percent_display">
										{if $product->specificPrice && $product->specificPrice.reduction_type == 'percentage'}-{$product->specificPrice.reduction*100}%{/if}
									</span>
								{/strip}</p>
								<p id="reduction_amount" class="col-xs-3 col-md-6" {if $productPriceWithoutReduction <= 0 || !$product->specificPrice || $product->specificPrice.reduction_type != 'amount' || $product->specificPrice.reduction|floatval ==0} style="display:none"{/if}>{strip}
...
{if (isset($quantity_discounts) && count($quantity_discounts) > 0)}
			...
			<section class="page-product-box">
				...
				<div id="combPriceImpacts" style="display:none;">
					{foreach $combPriceImpacts as  $k=>$v}
						<div id="{$k}" value="{$v.price}"></div>
					{/foreach}
				</div>
				<div id="quantityDiscount">
					<table class="std table-product-discounts">
						...
						{foreach from=$quantity_discounts item='quantity_discount' name='quantity_discounts'}
							...
							{foreach $combPriceImpacts as $k=>$v}
								{$realDiscountPrice=$initPrice|floatval + $v.price|floatval - $quantity_discount.real_value|floatval}
								<tr id="quantityDiscount_{$k}" class="quantityDiscount_{$k}" data-real-discount-value="{convertPrice price = $realDiscountPrice}" data-discount-type="{$quantity_discount.reduction_type}" data-discount="{$quantity_discount.real_value|floatval}" data-discount-quantity="{$quantity_discount.quantity|intval}">
									<td>
		...
		{if isset($product) && $product->description}
			...
			<section class="page-product-box">
				...
				<!-- full description aaaa-->
				{*modify by zhouzhiqiang*}
				{*<div  class="rte">{$product->description}</div>*}
				<div  class="rte mobile-indent"><script type="text/template" id="tpl_detailwindow">{$product->description}</script><iframe id="detailwindow" width="100%" frameborder="0" ></iframe>
					<script>
						var detailIframe = document.getElementById("detailwindow");
						subw = detailIframe.contentWindow, timerCount = 50;
						subw.document.write('<!DOCTYPE html>');
						subw.document.write('<html><head><meta name="viewport" content="width=device-width, initial-scale=1"><link href="/themes/es_mobile_wish/css/adjust-img.css" rel="stylesheet" /> </head><body>');
						var html = document.getElementById("tpl_detailwindow").innerHTML;
						...
						var timer = setInterval(function() {
							timerCount--;
							detailIframe.height = Math.max(subw.document.body ? subw.document.body.scrollHeight : 0, subw.document.documentElement.scrollHeight);
							if (timerCount <= 0) {
								clearInterval(timer)
							}
						}, 100);
							...
							{foreach from=$accessories item=accessory name=accessories_list}
								{if ($accessory.allow_oosp || $accessory.quantity_all_versions > 0 || $accessory.quantity > 0) && $accessory.available_for_order && !isset($restricted_country_mode)}
									...
									<li class="item product-box ajax_block_product{if $smarty.foreach.accessories_list.first} first_item{elseif $smarty.foreach.accessories_list.last} last_item{else} item{/if} product_accessories_description">
										...
										{*ccc: 屏蔽Accessories页的Add to cart按钮*}
