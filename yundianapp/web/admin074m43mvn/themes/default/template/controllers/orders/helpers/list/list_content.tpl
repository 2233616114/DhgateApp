...
{if count($list)}
    {foreach $list AS $index => $tr}
        ...
        <tr{if $position_identifier} id="tr_{$position_group_identifier}_{$tr.$identifier}_{if isset($tr.position['position'])}{$tr.position['position']}{else}0{/if}"{/if} class="{if isset($tr.class)}{$tr.class}{/if} {if $tr@iteration is odd by 1}warning{else}warning{/if}" {if isset($tr.color) && $color_on_bg} style="background-color: {$tr.color}"{/if} >
                    ...
                    {foreach $tr.products AS $pi => $product}
                        <tr>
                            ...
                            <td class="productQuantity text-center">
                                <span class="product_quantity_show badge">{(int)$product['product_quantity'] - (int)$product['customized_product_quantity']}</span>
                            </td>
                            <td class="unit_price_tax_excl">
                                <span>{displayPrice price=$product.unit_price_tax_incl currency=$currency->id}</span>
                            </td>
