...
<div id="product-prices" class="panel product-tab">
	...
	{*批发价*}
	<div class="form-group">
		<label class="control-label col-lg-2">{l s='Product price'}:</label>
		<div class="col-lg-4">
			<table id="batch_price_list" class="table">
				<thead>
				<tr>
					<th><input type="button" id="add_batch_price" class="btn btn-primary" value="  {l s='Add'}  "></th>
					<th></th>
					<th></th>
					<th></th>
					<th></th>
				...
				{foreach from=$specific_prices item=specific_price}
					<tr>
						<td class="control-label">{l s='Quantity'}：</td>
						<td><input type="text" name="batch_qty[]" value="{$specific_price.from_quantity}" class="form_input" size="5" maxlength="5" rel="amount"></td>
						<td class="control-label">{l s='Prices'}：{$currency->prefix}{$currency->suffix}</td>
						<td><input type="text" name="batch_price[]" value="{{toolsConvertPrice price=$specific_price.price}|string_format:'%.2f'}" class="form_input" size="5" maxlength="10" rel="amount"></td>
						<td><a href="javascript:;" onclick="document.getElementById('batch_price_list').deleteRow(this.parentNode.parentNode.rowIndex);"><i class="icon-trash"></i></a></td>
					</tr>
				{/foreach}
				</tbody>
			</table>
...
<script type="text/javascript">
	$('#add_batch_price').click(function () {
        if ($(this).parents('table').find('tr').size() > 5) return false;
        var newrow = document.getElementById('batch_price_list').insertRow(-1);
        newcell = newrow.insertCell(-1);
        newcell.setAttribute("class", "control-label");
		newcell.setAttribute("style", "padding-top: 15px");
        newcell.innerHTML = '{l s='Quantity'}：';
        newcell = newrow.insertCell(-1);
        newcell.innerHTML = '<input type="text" name="batch_qty[]" value="" class="form_input" size="5" maxlength="5" rel="amount">';
        newcell = newrow.insertCell(-1);
        newcell.setAttribute("class", "control-label");
		newcell.setAttribute("style", "padding-top: 15px");
        newcell.innerHTML = '{l s='Prices'}：{$currency->prefix}{$currency->suffix}';
        newcell = newrow.insertCell(-1);
        newcell.innerHTML = '<input type="text" name="batch_price[]" value="" class="form_input" size="5" maxlength="10" rel="amount">';
        newcell = newrow.insertCell(-1);
        newcell.innerHTML = '<a href="javascript:;" onclick="document.getElementById(\'batch_price_list\').deleteRow(this.parentNode.parentNode.rowIndex);"><i class="icon-trash"></i></a>';
	});
</script>
