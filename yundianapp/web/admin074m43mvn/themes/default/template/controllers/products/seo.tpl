...
<div id="product-seo" class="panel product-tab">
	...
	<div class="panel-footer" style="position:fixed;bottom:20px;left:240px;right:0px;width:85%;z-index: 999999">
		<a href="{$link->getAdminLink('AdminProducts')|escape:'html':'UTF-8'}{if isset($smarty.request.page) && $smarty.request.page > 1}&amp;submitFilterproduct={$smarty.request.page|intval}{/if}" class="btn btn-warning pull-right"><i class="process-icon-cancel"></i> {l s='Cancel'}</a>
		{*{if isset($product->id)}*}
		...
		{*{else}*}
			{*<button type="submit" name="submitAddproductAndStay" class="btn btn-primary pull-right" disabled="disabled"><i class="process-icon-loading"></i> {l s='Save'}</button>*}
		{*{/if}*}
		<button type="submit" name="submitAddproduct" class="btn btn-primary pull-right"><i class="process-icon-loading"></i> {l s='Save'}</button>
	</div>
</div>
