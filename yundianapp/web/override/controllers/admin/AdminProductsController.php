...
//            'Quantities' => $this->l('Quantities'),
            'Productstock' => $this->l('Quantities'),
            'Suppliers' => $this->l('Suppliers'),
            'Warehouses' => $this->l('Warehouses'),
            'Specs' => $this->l('Combinations'),
        );

        $this->available_tabs = array('Productstock' => 3, 'Warehouses' => 14);
        //$this->available_tabs = array('Quantities' => 3,'Productstock' => 3, 'Warehouses' => 14);
...
//                'Attachments' => 12,
    ...
    public function setMedia()
    {
        ...
        // JQuery拖拽排序
        $this->addJs(__PS_BASE_URI__.'js/jquery/jquery.dragsort-0.5.2.min.js');
        $this->addJs(__PS_BASE_URI__.'js/jquery/ui/jquery.ui.draggable.min.js');
        // $this->addJs(__PS_BASE_URI__.'js/jquery/ui/jquery-ui.min.js');
    }
     ...
     * 处理产品规格组合
     */
    public function processProductSpecs()
    {
        $ret = false;
        if (!Validate::isLoadedObject($product = $this->object)) {
            return false;
        ...
        if ($this->tabAccess['edit'] === '1') {
            ...
            // 获取规格参数列表
            $attrCombs = Tools::getValue('attr_combination');
            $specsRefs = Tools::getValue('specs_ref');
            $specsPrices = Tools::getValue('specs_price');
            $specsWeights = Tools::getValue('specs_weight');
            $specsMinQtys = Tools::getValue('specs_min_qty');
            $specsStockQtys = Tools::getValue('specs_stock_qty');
            // 逐个增加
            $count = count($attrCombs);
            $defaultProductAttributeId = 0;
            $totalStockQty = 0;
            for ($x = 0; $x < $count; $x++) {
                $attrComb = $attrCombs[$x];
                $specsRef = $specsRefs[$x];
                $specsPrice = $specsPrices[$x];
                $specsWeight = $specsWeights[$x];
                $specsMinQty = $specsMinQtys[$x];
                $specsStockQty = $specsStockQtys[$x];
                $specsImages =  Tools::getValue('specs_images_'.$attrComb);
                $totalStockQty += $specsStockQty;

                if ($attrComb == null || $attrComb == '') {
                    $totalStockQty = -1;
                    continue;
                }
                ...
                $id_product_attribute = $product->addCombinationEntity(
                    ...
                    $specsPrice, //Tools::getValue('attribute_price') * Tools::getValue('attribute_price_impact'),
                    $specsWeight, //Tools::getValue('attribute_weight') * Tools::getValue('attribute_weight_impact'),
                    0, //Tools::getValue('attribute_unity') * Tools::getValue('attribute_unit_impact'),
                    0, //Tools::getValue('attribute_ecotax'),
                    0,
                    $specsImages,//array(), //Tools::getValue('id_image_attr'),
                    $specsRef, //Tools::getValue('attribute_reference'),
                    null,
                    '', //Tools::getValue('attribute_ean13'),
                    0, //Tools::getValue('attribute_default'),
                    '', //Tools::getValue('attribute_location'),
                    '', //Tools::getValue('attribute_upc'),
                    $specsMinQty, //Tools::getValue('attribute_minimal_quantity'),
                    array(),
                    '0000-00-00' //Tools::getValue('available_date_attribute')
                );
                // 更新库存
                StockAvailable::setProductDependsOnStock((int)$product->id, $product->depends_on_stock, null, (int)$id_product_attribute);
                StockAvailable::setProductOutOfStock((int)$product->id, $product->out_of_stock, null, (int)$id_product_attribute);
                StockAvailable::setQuantity((int)$product->id, (int)$id_product_attribute, (int)$specsStockQty);
                // 设置规格组合
                $combination = new Combination((int)$id_product_attribute);
                $combination->setAttributes(explode("_", $attrComb));
                // 设置规格图片
                $combination->setImages($specsImages);
                // 第1个为默认规格
                if ($x == 0) {
                    $defaultProductAttributeId = $id_product_attribute;
                }
            }
            // 更新总库存
            if ($totalStockQty != -1) {
                $ret = true;
                StockAvailable::setQuantity((int)$product->id, (int)0, (int)$totalStockQty);
            }
            // 设置默认规格
            if ($defaultProductAttributeId != 0) {
                $product->deleteDefaultAttributes();
                $product->setDefaultAttribute($defaultProductAttributeId);
            }
        } else {
            $this->errors[] = Tools::displayError('You do not have permission to add this.');
        }
     ...
     * 处理产品库存更新
     */
    public function processProductStockUpdate()
    {
        if (!Validate::isLoadedObject($product = $this->object)) {
            return;
        }
        if ($this->tabAccess['edit'] === '1') {
            // 获取产品库存
            $productStock = Tools::getValue('productStock');
            StockAvailable::setQuantity((int)$product->id, (int)0, (int)$productStock);
     ...
     * 批量增加批发价
     */
    public function processBatchSpecificPriceAddition($product)
    {
        $id_product = Tools::getValue('id_product');
//        // 查询规格组合
//        $currency = $this->context->currency;
//        $specCombs = $this->querySpecCombinations($product, $currency);

        // 删除改产品的所有 specific price
        $this->deleteSpecificPrices($id_product);

        $batch_qty = Tools::getValue('batch_qty');
        $batch_price = Tools::getValue('batch_price');

        // 批量增加 specific price
        $count = count($batch_qty);
        for($x=0; $x<$count; $x++) {
            // 批发数量
            $from_quantity = (int)($batch_qty[$x]);
            // 批发价格
            $price = (float)($batch_price[$x]);
            if ($from_quantity <= 0 || $price < 0) {
                continue;
            }
            ...
            $specificPrice->id_product = (int)$id_product;
            $specificPrice->id_product_attribute = (int)0;
            $specificPrice->id_shop = (int)0;
            $specificPrice->id_currency = (int)(0);
            $specificPrice->id_country = (int)(0);
            $specificPrice->id_group = (int)(0);
            $specificPrice->id_customer = (int)0;
            $specificPrice->reduction = (float)0.00;
...
//            // 如果有规格设置，且有价格变化，则需要新增每个规格的批发价
//            foreach ($specCombs as $k => $specComb) {
//                $priceImpact = $specComb['price'];
//                if ($priceImpact != 0) {
//                    $specificPrice->id_product_attribute = $k;
//                    $specificPrice->price = $price + $priceImpact;
//                    $specificPrice->add();
//                }
//            }
...
//        }
        ...
        if (!$priorities = Tools::getValue('specificPricePriority')) {
            $priorities = array("id_shop", "id_currency", "id_country", "id_group");
        }
    ...
    public function postProcess()
    {
        ...
        if ($this->display == 'edit' || $this->display == 'add') {
            ...
            $this->addJS(array(
                ...
                //  ===== add by ccc @ 20170320 =======
                _PS_JS_DIR_.'admin/specs/specs_v0.3.js',
                // ====================================
     ...
     * 只需要删除临时目录下的文件即可
     */
    public function ajaxProcessDeleteProductImageTemp()
    {
        $imagePath = Tools::getValue('imagePath');
        $imagePath = _PS_ROOT_DIR_.$imagePath;
        if (file_exists($imagePath)) {
            @unlink($imagePath);
        }
        // 删除临时的小图片文件
        $imageSmallName = substr($imagePath, 0, strrpos ($imagePath, ".")).'-small_default.jpg';
        if (file_exists($imageSmallName)) {
            unlink($imageSmallName);
        }
        $imageTemp = substr($imagePath, 0, strrpos ($imagePath, "."));
...
//        $this->object = $this->loadObject();
        ...
        // 增加产品图片
        $this->addProductImagesAferAddAction($this->object);
    ...
    public function processUpdate()
    {
        ...
        if (!empty($this->errors)) {
            PrestaShopLogger::addLog('processUpdate error: '.$this->errors[0]);
            $this->display = 'edit';
            return false;
        }

        // modify by ccc 为支持一页增加产品
        // $id = (int)Tools::getValue('id_'.$this->table);
        $id = $existing_product->id;
        // 设置id_product给$POST
        Tools::setValue('id_'.$this->table, $id);
...
//                        }
                        // 处理规格组合
                        $productTotalStockUpdated = false;
                        if ($this->isTabSubmitted('Specs')) {
                            $productTotalStockUpdated = $this->processProductSpecs();
                        }
                        if ($this->isTabSubmitted('Prices')) {
                            // 屏蔽原有的specific price
                            // $this->processPriceAddition();
                            // 处理批发价  ccc
                            $this->processBatchSpecificPriceAddition($object);
                            //
                        ...
                        // 更新库存数量
                        if ($this->isTabSubmitted('Productstock') && !$productTotalStockUpdated) {
                            $this->processProductStockUpdate();
                        }
                        ...
                        // ccc: 更新图片cover
                        $this->updateImageCover($object->id);
                    }
            ...
            $this->errors[] = 'product id not exist';
        }
    }
     ...
     * @param $id_product
     */
    public function updateImageCover($id_product) {
        $imageIds = Tools::getValue("imageIds");
        $id_image = -1;
    ...
    */
    ...
    public function initFormPrices($obj)
    {
        ...
        if ($obj->id) {
            ...
            // add by ccc @ 201703
            // 批发价
            $data->assign('specific_prices', SpecificPrice::getByProductId((int)$obj->id));
...
//            $this->displayWarning($this->l('You must save this product before adding attachements.'));
     ...
     * 图片保存到临时目录下
     */
    public function ajaxProcessaddProductImageTemp()
    {
        self::$currentIndex = 'index.php?tab=AdminProducts';
        ...
        foreach ($files as $k => &$file) {
            $file['id'] = 0;
            $imageNameWithPath = $file['save_path'];
            // 获取文件路径
            $imagePath = substr($imageNameWithPath, 0, strrpos ($imageNameWithPath, DIRECTORY_SEPARATOR)).DIRECTORY_SEPARATOR;
            // 获取文件名
            $imageName = substr(strrchr($imageNameWithPath, DIRECTORY_SEPARATOR), 1);
            // 将原始图片resize为 name-small_default.jpg
            $imageSmallName = substr($imageName, 0, strrpos ($imageName, ".")).'-small_default.jpg';
            $imageSmallNameWithPath = $imagePath.$imageSmallName;
            $error = 0;
            ImageManager::resize($imageNameWithPath, $imageSmallNameWithPath, 98, 98, 'jpg', false, $error);
            // 赋值
            $file = array_merge($file, array("path" => $imageName));
            $file = array_merge($file, array("imageSmallName" => $imageSmallName));
            $file = array_merge($file, array("position" => $position++));
            $file = array_merge($file, array("id" => "0"));
            $file = array_merge($file, array("shops" => ""));
            $file = array_merge($file, array("status" => "ok"));
        }
        die(Tools::jsonEncode(array($image_uploader->getName() => $files)));
    }
     ...
     * 在增加产品时，增加产品图片 ccc
     */
    public function addProductImagesAferAddAction($product) {
        $imageNames = Tools::getValue("imageNames");
        $position = 1;
        foreach ($imageNames as $imageName) {
            if ($imageName == 'image_path') {
                // 过滤掉hidden属性的imageNames
                continue;
            }
            $imagePath = _PS_TMP_IMG_UPLOAD_DIR_.$imageName;
//            PrestaShopLogger::addLog('imagePath:'.$imagePath);
...
//            }
            ...
            if (!$image->add()) {
                $file['error'] = Tools::displayError('Error while creating additional image');
                PrestaShopLogger::addLog('Error while creating additional image: '.$imagePath);
            } else {
                if (!$new_path = $image->getPathForCreation()) {
                    $file['error'] = Tools::displayError('An error occurred during new folder creation');
                    PrestaShopLogger::addLog('An error occurred during new folder creation: '.$imagePath);
                    continue;
                }
                $error = 0;
                if (!ImageManager::resize($imagePath, $new_path.'.'.$image->image_format, null, null, 'jpg', false, $error)) {
                    PrestaShopLogger::addLog('image resize error, error code='. $error.', '.$imagePath);
                    switch ($error) {
                        case ImageManager::ERROR_FILE_NOT_EXIST :
                            $file['error'] = Tools::displayError('An error occurred while copying image, the file does not exist anymore.');
                            break;
...
//                    PrestaShopLogger::addLog('image resize:'.$new_path.'.'.$image->image_format);

                    $imagesTypes = ImageType::getImagesTypes('products');
                    $generate_hight_dpi_images = (bool)Configuration::get('PS_HIGHT_DPI');

                    foreach ($imagesTypes as $imageType) {
                        if (!ImageManager::resize($imagePath, $new_path.'-'.stripslashes($imageType['name']).'.'.$image->image_format, $imageType['width'], $imageType['height'], $image->image_format)) {
                            $file['error'] = Tools::displayError('An error occurred while copying image:').' '.stripslashes($imageType['name']);
                            continue;
                        }

                        if ($generate_hight_dpi_images) {
                            if (!ImageManager::resize($imagePath, $new_path.'-'.stripslashes($imageType['name']).'2x.'.$image->image_format, (int)$imageType['width']*2, (int)$imageType['height']*2, $image->image_format)) {
                                $file['error'] = Tools::displayError('An error occurred while copying image:').' '.stripslashes($imageType['name']);
                                continue;
                            }
                        }
    ...
    public function initFormImages($obj)
    {
        ...
        if ((bool)$obj->id) {
            ...
            $image_uploader->setMultiple(!(Tools::getUserBrowser() == 'Apple Safari' && Tools::getUserPlatform() == 'Windows'))
                ->setUseAjax(true)->setUrl(
                    Context::getContext()->link->getAdminLink('AdminProducts').'&ajax=1&action=addProductImageTemp');
     ...
     * @param $obj
     */
    public function initFormSpecs($product)
    {
        $data = $this->createTemplate($this->tpl_form);
        $currency = $this->context->currency;
        $data->assign('currency', $currency);
        $data->assign($this->tpl_form_vars);
        $data->assign('weight_unit', Configuration::get('PS_WEIGHT_UNIT'));
        $data->assign(array(
            'product' => $product,
        ));
        // 设置images
        ...
        $data->assign('attribute_groups', $attribute_groups);
        $data->assign('attribute_js', $attribute_js);
        // 查询规格组合
        $specCombs = $this->querySpecCombinations($product, $currency);
        $data->assign('spec_combs', $specCombs);
        // 获取已选择的规格属性
        $selectedAttrs = array();
        foreach ($specCombs as $k => $specComb) {
            $comdId = $specComb[comb_id];
            $comdIdArray = explode('_', $comdId);
            foreach ($comdIdArray as $k => $selectedAttr) {
                $selectedAttrs[$selectedAttr] = '1';
            }
        }
        $data->assign('selectedAttrs', $selectedAttrs);
        //
        $this->tpl_form_vars['custom_form'] = $data->fetch();
    }
     ...
     * @return string
     */
    public function querySpecCombinations($product, $currency)
    {
        ...
        if (is_array($combinations)) {
            ...
            foreach ($combinations as $k => $combination) {
                $price = Tools::convertPrice($combination['price'], $currency);
//                $price = Tools::displayPrice($price, $currency);
                ...
                $comb_array[$combination['id_product_attribute']]['price'] = Tools::ps_round($price, 2);
                $comb_array[$combination['id_product_attribute']]['price_unit'] = Tools::ps_round($price, 2);
                $comb_array[$combination['id_product_attribute']]['weight'] = Tools::ps_round($combination['weight'], 2);
                ...
                // 查询库存
                $comb_array[$combination['id_product_attribute']]['stock_qty'] = StockAvailable::getQuantityAvailableByProduct($product->id, $combination['id_product_attribute']);
                // 设置图片
                $combImages = $combination_images[$combination['id_product_attribute']];
                $combImageIds = array();
                foreach ($combImages as $k => $v) {
                    array_push($combImageIds, $v['id_image']);
                }
                $comb_array[$combination['id_product_attribute']]['specs_images'] = implode("_", $combImageIds);
                $comb_array[$combination['id_product_attribute']]['specs_images_array'] = $combImageIds;
            }
        }
        ...
        if (isset($comb_array)) {
            foreach ($comb_array as $id_product_attribute => $product_attribute) {
                ...
                foreach ($product_attribute['attributes'] as $attribute) {
                    $listCombId .= $attribute[2].'_';
                    $listCombDesc .= $attribute[1].' + ';
                }
                $listCombId = rtrim($listCombId, '_');
                $listCombDesc = rtrim($listCombDesc, ' + ');
                ...
                $comb_array[$id_product_attribute]['comb_id'] = $listCombId;
                $comb_array[$id_product_attribute]['comb_desc'] = $listCombDesc;
            }
        }
...
//            $this->displayWarning($this->l('You must save this product before managing quantities.'));
     ...
     * @throws SmartyException
     */
    public function initFormProductStock($obj)
    {
        if (!$this->default_form_language) {
            $this->getLanguages();
        }
        ...
        if ($obj->id) {
            if ($this->product_exists_in_shop) {
                ...
                if ($countAttributes == 0) {
                    $showQtyPage = true;
                    $productStock = StockAvailable::getQuantityAvailableByProduct($obj->id, 0);
                }
            } else {
                $showQtyPage = false;
            }
        ...
        $data->assign('showQtyPage', $showQtyPage);
        if ($showQtyPage) {
            $data->assign(array(
                'product' => $obj,
...
//    }
     ...
     * @return bool
     */
    public function deleteSpecificPrices($id_product)
    {
        // Delete product specific prices
