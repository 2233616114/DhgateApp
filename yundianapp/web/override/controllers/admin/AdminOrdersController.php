...
//		LEFT JOIN `'._DB_PREFIX_.'order_state_lang` osl ON (os.`id_order_state` = osl.`id_order_state` AND osl.`id_lang` = '.(int)$this->context->language->id.')';
		...
		osl.`name` AS `osname`,
		os.`color`,
		CONCAT (c.firstname, \' \', c.lastname, \', \', address.address1, \' \', address.address2, \', \', address.postcode, \' \', address.city, \', \', country_lang.name) as cname,
		IF(a.valid, 1, 0) badge_success';
...
//            );
	...
	// modify by Jacky@20170301
        //$this->addJS('https://maps.googleapis.com/maps/api/js?v=3.exp&sensor=false');
        $this->addJS(_PS_JS_DIR_.'google/maps.googleapis.com.js');
    ...
    /**
     * 获取订单列表后进行进一步的处理，包括获取订单下的产品列表等  (Add by Jacky@20170301)
     */
    public function postProcessGetList() {
        foreach ($this->_list as &$orderRecord) {
            $order = new Order($orderRecord['id_order']);
            $products = $this->getProducts($order);
