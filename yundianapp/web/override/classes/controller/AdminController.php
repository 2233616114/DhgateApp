        ...
        switch ($this->display) {
            case 'view':
                ...
                if (Validate::isLoadedObject($obj) && isset($obj->{$this->identifier_name}) && !empty($obj->{$this->identifier_name})) {
                    ...
                    $default_language = 1;
                    $this->toolbar_title[] = is_array($obj->{$this->identifier_name}) ? $obj->{$this->identifier_name}[$default_language] : $obj->{$this->identifier_name};
                    //
            ...
            case 'edit':
                ...
                if (Validate::isLoadedObject($obj) && isset($obj->{$this->identifier_name}) && !empty($obj->{$this->identifier_name})) {
                    ...
                    $this->toolbar_title[] = sprintf($this->l('Edit: %s'), (is_array($obj->{$this->identifier_name}) && isset($obj->{$this->identifier_name}[$default_language])) ? $obj->{$this->identifier_name}[$default_language] : $obj->{$this->identifier_name});
                    //
                    $this->addMetaTitle($this->toolbar_title[count($this->toolbar_title) - 1]);
                }
                break;
...
//        }
    ...
    {
        // PrestaShopLogger::addLog('ccc initProcess ...');
        if (!isset($this->list_id)) {
            $this->list_id = $this->table;
        }
...
//            PrestaShopLogger::addLog('ccc initProcess submitAdd ...');
            // case 1: updating existing entry
            if ($this->id_object) {
//                PrestaShopLogger::addLog('ccc initProcess submitAdd update 1');
                if ($this->tabAccess['edit'] === '1') {
//                    PrestaShopLogger::addLog('ccc initProcess submitAdd update 2');
                    $this->action = 'save';
                    if (Tools::isSubmit('submitAdd'.$this->table.'AndStay')) {
                        $this->display = 'edit';
                    } else {
...
//                    PrestaShopLogger::addLog('ccc initProcess submitAdd update error: You do not have permission to edit this');
                    $this->errors[] = Tools::displayError('You do not have permission to edit this.');
                }
            } else {
                // case 2: creating new entry
                // PrestaShopLogger::addLog('ccc initProcess submitAdd new');
                if ($this->tabAccess['add'] === '1') {
                    // PrestaShopLogger::addLog('ccc initProcess submitAdd new 1');
                    $this->action = 'save';
                    if (Tools::isSubmit('submitAdd'.$this->table.'AndStay')) {
                        $this->display = 'edit';
                    } else {
                    ...
                    // PrestaShopLogger::addLog('ccc initProcess submitAdd new error: You do not have permission to add this.');
    ...
    {
        // important ccc, force list language to english
        $id_lang = 1;
    ...
    public function postProcessGetList() {
    }
